...
function customPageCode_Var1() {
    ...
    var top, attachOnce = true;

    $(window).resize(function () {
        if ($('#buyers-guide').length) {
            top = $('#buyers-guide').offset().top;
        }
    });

    $(document).on('click', '.cxl-sticky-back a', function(e) {
        e.preventDefault();

        $('#view-products-tab-sem').click();
    ...
    $(document).on('click', '.customBuyingGuidContent .table-of-contents li a[href^="#"]', function (e) {
        e.preventDefault();
    ...
    $('.customTabTitle ul li').click(function() {
        $(this).siblings().removeClass('active');
        $(this).addClass('active');

        var getindex = $(this).index();
        $('#sem-landing-page-original > div').removeClass('active');
        $('#sem-landing-page-original > div').eq(getindex).addClass('active');
        $('.cxl-sticky-back').removeClass('is-sticky');
        ...
        if (attachOnce) {
            ...
            $(window).scroll(function (event) {
                ...
                if (y >= top) {
                    $('.cxl-sticky-back').addClass('is-sticky');
                } else {
                    $('.cxl-sticky-back').removeClass('is-sticky');
                }
            });
    ...
    $('#sem-landing-page-original .stack-content').addClass('active');
    //Create wrapper for second tab content
    $('#sem-landing-page-original').append('<div class="customBuyingGuidContent"></div>');
    //Load content form another page for second tab
    var currentUrl = location.pathname.replace('/sem','');
    $('.customBuyingGuidContent').load(currentUrl + '#buyers-guide' + ' #buyers-guide', function(response, status) {
        if (status === 'success') {
            if (!$('.cxl-sticky-back').length) {
                $('#sem-landing-page-original .customBuyingGuidContent').prepend('<div class="cxl-sticky-back sticky"><a href="#"><< Back to Product List</a></div>');

                if ($('.site-logo-wrapper>a img').length) {
                    var logo = $('.site-logo-wrapper>a').html();

                    if (!$('.cxl-sticky-back img').length) {
                        $('.cxl-sticky-back').prepend(logo);
                    }
                }
            }
        }
